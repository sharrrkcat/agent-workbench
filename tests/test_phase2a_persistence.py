...
def test_unified_store_crud_references_and_secret_redaction(tmp_path, memory):
    ...
    with TestClient(app) as client:
        ...
        provider_id = profile["source"]["provider_profile_id"]
        assert "provider-private-key" not in client.get("/api/models/providers").text
        assert client.get(f"/api/models/providers/{provider_id}").json()["connection"]["has_api_key"]
...
def test_unbound_nonchat_models_are_first_class_but_require_a_source(tmp_path, kind):
    with TestClient(create_app(use_memory=True, root=tmp_path)) as client:
        ...
        assert unavailable.json()["error"]["code"] == "MODEL_NOT_CONFIGURED"
