...
export function PresetVoices({ profileId }: { profileId: string }) {
  ...
  return (
    <section aria-label={t('voices.title')}>
      <div className="tts-voices-heading">
        <h3>{t('voices.title')}</h3>
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
      ...
      <div className="tts-voices-list">
        <table>
          <thead>
            <tr>
              <th>{t('voices.id')}</th>
              <th>{t('voices.availability')}</th>
            </tr>
          </thead>
          {languages.map((language) => (
            <tbody key={language}>
              <tr>
                <th colSpan={2}>{t('voices.languages.' + language)}</th>
              </tr>
              {voices
                ...
                .map((voice) => (
                  <tr key={voice.id}>
                    ...
                    <td>{voice.available ? t('voices.available') : t('voices.missing')}</td>
                  </tr>
                ))}
            </tbody>
          ))}
