...
export const entryInput = (entry?: WorldbookEntryInput): WorldbookEntryInput => ({
  ...
  activation_mode: entry?.activation_mode ?? 'keyword',
  enabled: entry?.enabled ?? true,
});

export function WorldbookEntries({
  bookId,
  onState,
  onCount,
}: {
  ...
  const dirty = Object.entries(drafts).some(
    ([id, draft]) => id === 'new' || !equalDraft(draft, entryInput(entries.find((entry) => entry.id === id))),
  );
  useEffect(() => {
    onState({ dirty, busy: !!busy || loading || !!dragging });
  }, [dirty, busy, loading, dragging, onState]);
  useEffect(() => {
    let live = true;
    setLoading(true);
    setLoadError('');
    void worldbookApi
      .listWorldbookEntries(bookId)
      .then((values) => {
        if (live) {
          setEntries(values);
          setLoading(false);
  ...
  function update(id: string, patch: Partial<WorldbookEntryInput>) {
    setDrafts((current) => ({
      ...current,
      [id]: { ...(current[id] || entryInput(entries.find((entry) => entry.id === id))), ...patch },
    }));
  }
  ...
  async function save(id: string) {
    ...
    await mutate(id, 'save', async () => {
      const saved =
        id === 'new'
          ? await worldbookApi.createWorldbookEntry(bookId, input)
          : await worldbookApi.patchWorldbookEntry(id, input);
      setEntries((current) =>
        id === 'new' ? [...current, saved] : current.map((item) => (item.id === id ? saved : item)),
      );
      clearDraft(id);
      setExpanded((current) => [...new Set([...current.filter((value) => value !== 'new'), saved.id])]);
      onCount(entries.length + (id === 'new' ? 1 : 0));
      setNotice(t('entrySaved'));
    });
  }
  async function enable(entry: WorldbookEntry, enabled: boolean) {
    await mutate(entry.id, 'toggle', async () => {
      ...
      try {
        ...
        setEntries((current) =>
          current.map((item) => (item.id === entry.id ? { ...item, enabled: entry.enabled } : item)),
        );
        update(entry.id, { enabled: entry.enabled });
        throw reason;
      }
    });
  }
  ...
  async function reorder(id: string, target: string) {
    ...
    await mutate(id, 'reorder', async () => {
      ...
      try {
        setEntries(
          (
            await worldbookApi.reorderWorldbookEntries(
              bookId,
  ...
  if (loading || loadError)
    return <ResourceLoading error={loadError} retry={() => setRevision((value) => value + 1)} />;
  const ids = [...(drafts.new ? ['new'] : []), ...entries.map((entry) => entry.id)];
  return (
    <div className="worldbook-entries">
      <div className="resource-toolbar">
        <h3>
          {t('entries')} <span className="resource-count">{entries.length}</span>
        </h3>
        <Button
          type="button"
          disabled={!!busy || !!drafts.new}
          onClick={() => {
            update('new', entryInput());
            setExpanded((current) => ['new', ...current]);
          }}
          variant="outline"
        >
        ...
        {ids.map((id) => {
          ...
          return (
            <WorldbookEntryCard
              ...
              dirty={id === 'new' || !equalDraft(draft, entryInput(entry))}
              busy={busy.endsWith(`:${id}`) ? busy.split(':')[0] : ''}
              locked={!!busy}
              ...
              onEnabled={(enabled) => (entry ? void enable(entry, enabled) : update(id, { enabled }))}
              onSave={() => void save(id)}
              onReset={() => clearDraft(id)}
              onDelete={() => remove(id)}
              onPointerDown={(event) => {
              ...
              onPointerMove={(event) => {
                ...
                if (target && target !== 'new') drag.current.target = target;
              }}
              onPointerUp={(event) => {
                ...
                if (active) void reorder(active.id, active.target);
              }}
              onPointerCancel={() => {
                drag.current = null;
                setDragging('');
