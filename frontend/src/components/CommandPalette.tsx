...
export function CommandPalette({ mode, input, onPick }: { mode: PaletteMode; input: string; onPick: (value: string) => void }) {
  const { agents, commands } = useWorkbenchStore();
  if (mode === 'none') return null;

  const actionAgentId = input.match(/^@([a-zA-Z][a-zA-Z0-9_-]*):/)?.[1];
