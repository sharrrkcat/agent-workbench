...
const fieldVariants = cva('group/field flex w-full gap-2 data-invalid:text-destructive', {
  variants: {
    orientation: {
      ...
      horizontal:
        'flex-row items-center pointer-coarse:min-h-11 pointer-coarse:*:data-[slot=field-label]:min-h-11 *:data-[slot=field-label]:items-center has-[>[data-slot=field-content]]:items-start *:data-[slot=field-label]:flex-auto has-[>[data-slot=field-content]]:[&>[role=checkbox],[role=radio]]:mt-px',
      responsive:
...
function Field({
  ...
  orientation = 'vertical',
  ...props
}: React.ComponentProps<'div'> & { disabled?: boolean } & VariantProps<typeof fieldVariants>) {
  return (
    <FieldPrimitive.Root
      role="group"
      data-slot="field"
