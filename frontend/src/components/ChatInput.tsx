...
export function ChatInput() {
  ...
  const eligible = messages.filter((m) => ['user', 'assistant', 'tool'].includes(m.role) && !m.metadata?.streaming && !m.metadata?.event_type && !m.parts.some((p) => p.type === 'error'));
  const hasSource = !!sourceMessageId && eligible.some((m) => m.message_id === sourceMessageId);

  async function submit() {
    if (sending || uploading || activeRun || (contextRequired && !hasSource) || (!draft.trim() && attachments.length === 0)) return;
    ...
    <div className="composer-wrap">
      ...
      <div className="composer">
        ...
        <textarea
          ...
          disabled={uploading}
        />
        {activeRun ? <button className="icon-button danger" type="button" disabled={activeRun.status === 'CANCELLING'} title={t('cancel')} aria-label={t('cancel')} onClick={() => void cancelRun(activeRun.run_id)}><Square size={16} /></button> : <button className="send-button" type="button" aria-label={t('send')} disabled={sending || uploading || (contextRequired && !hasSource) || (!draft.trim() && attachments.length === 0)} onClick={() => void submit()}><Send size={17} /></button>}
      </div>
    </div>
  );
}
