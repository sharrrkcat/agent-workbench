  ...
  for (const viewport of [{ width: 1366, height: 900 }, { width: 390, height: 844 }]) {
    test.describe(`TTS ${locale} ${viewport.width}`, () => {
      ...
      test('configure ONNX TTS, reopen voices and retain format', async ({ page, request }, info) => {
        ...
        await fillCombobox(dialog.getByLabel(locale === 'en' ? 'Model reference' : '模型引用', { exact: true }), 'tts/kokoro');
        await expect(dialog.getByText('kokoro', { exact: true })).toBeVisible();
        await chooseOption(dialog.getByLabel(locale === 'en' ? 'Audio format' : '音频格式', { exact: true }), 'WAV');
        ...
        const overflow = await page.evaluate(() => [...document.querySelectorAll('body, [data-slot="dialog-content"], .model-form, .tts-voices-list')]
