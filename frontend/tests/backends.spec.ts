...
for (const locale of ['en', 'zh-CN']) {
  ...
  test(`local singleton and external backend key editing (${locale})`, async ({ page, request }) => {
    await page.addInitScript((value) => localStorage.setItem('agent-workbench.locale', value), locale);
    await page.goto('/settings?tab=models');
    await expect(page.locator('.model-tabs [role="tab"]')).toHaveCount(3);
    await page.getByRole('tab', { name: labels.backends, exact: true }).click();
    const local = page.locator('.model-row').filter({ has: page.getByText(labels.localBackend, { exact: true }) });
    await expect(local).toHaveCount(1);
    await expect(local.getByRole('button', { name: labels.delete, exact: true })).toHaveCount(0);
    await expect(local.getByRole('button', { name: labels.edit, exact: true })).toHaveCount(0);
    await page.getByRole('button', { name: labels.addBackend, exact: true }).click();
    ...
    let backend = (await (await request.get('/api/models/backends')).json()).find((item: { name: string }) => item.name === name);
    expect(backend.connection.has_api_key).toBe(true);
    expect(backend.connection).not.toHaveProperty('api_key');
    ...
    const preserved = page.waitForRequest((value) => value.method() === 'PATCH' && value.url().endsWith(`/backends/${backend.id}`));
    await dialog.getByRole('button', { name: labels.save, exact: true }).click();
    expect((await preserved).postDataJSON().connection).not.toHaveProperty('api_key');
    await expect(dialog).toHaveCount(0);
    backend = await (await request.get(`/api/models/backends/${backend.id}`)).json();
    expect(backend.connection.has_api_key).toBe(true);
    expect(backend.connection.timeout_seconds).toBe(90);
    await row.getByRole('button', { name: labels.edit, exact: true }).click();
    await dialog.getByLabel(labels.apiKey, { exact: true }).fill('clear');
    await dialog.getByLabel(labels.apiKey, { exact: true }).fill('');
    await dialog.getByRole('button', { name: labels.save, exact: true }).click();
    await expect(dialog).toHaveCount(0);
    expect((await (await request.get(`/api/models/backends/${backend.id}`)).json()).connection.has_api_key).toBe(false);
