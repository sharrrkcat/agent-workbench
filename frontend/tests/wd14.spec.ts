  ...
  for (const viewport of [{ width: 1366, height: 900 }, { width: 390, height: 844 }]) {
    test.describe(`WD14 ${locale} ${viewport.width}`, () => {
      ...
      test('local inventory, thresholds, save and lifecycle controls', async ({ page, request }, info) => {
        ...
        await expect(dialog.getByRole('group', { name: labels.directory.information, exact: true })).toContainText('wd14');
        ...
        expect(saved.parameters).toEqual({ task: 'tags', thresholds: { general: 0, character: 0.875 } });
