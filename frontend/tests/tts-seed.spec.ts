    ...
    test.describe(`Speech seed ${locale} ${viewport.width}`, () => {
      ...
      test('save zero, clear seed and reset on architecture changes', async ({ page, request }, info) => {
        ...
        for (const selected of ['chatterbox', 'qwen3tts']) {
          ...
          await dialog.getByLabel(labels.modelRef, { exact: true }).fill(`tts/fixture-${selected}`);
          await seed.fill('0');
          await seed.scrollIntoViewIfNeeded();
          const overflow = await page.evaluate(() => [...document.querySelectorAll('body, .app-modal-panel, .model-form-grid')]
          ...
          await expect(seed).toHaveValue('');
          await seed.fill('123');
          await selector.selectOption(selected === 'chatterbox' ? 'qwen3tts' : 'chatterbox');
          await expect(seed).toHaveValue('');
          await selector.selectOption('kokoro');
          await expect(seed).toHaveCount(0);
