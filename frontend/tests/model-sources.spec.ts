...
for (const locale of ['en', 'zh-CN']) {
  ...
  test(`source filtering and stale discovery (${locale})`, async ({ page, request }) => {
    ...
    for (const selected of ['embedding', 'tts', 'reranker', 'image_embedding', 'vision']) {
      ...
      await page.getByRole('button', { name: labels.addModel, exact: true }).click();
      await expect(source.locator('[data-slot="select-value"]')).toHaveText(selected === 'tts' ? labels.localRuntime : labels.unbound);
      await source.click();
      await expect(page.getByRole('listbox')).toBeVisible();
      await expect(page.getByRole('option', { name: labels.localRuntime, exact: true })).toHaveCount(selected === 'tts' ? 1 : 0);
