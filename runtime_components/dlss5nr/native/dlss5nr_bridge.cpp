...
static std::wstring g_caller_path;
static std::wstring g_work_dir;
...
static HMODULE LoadCoreNGX() {
    if (HMODULE m = LoadLibraryExW(L"_nvngx.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32)) return m;
...
static bool LoadNGX() {
    g_core_mod = LoadCoreNGX();
    if (!g_core_mod) {
        SetError("Could not load NVIDIA NGX core from the installed display driver.");
        return false;
    }
    ...
    const std::wstring& shim_path = g_caller_path;
...
static bool InitNGXSession() {
    ...
    if (g_core_init_project) {
        for (int ver = 0x13; ver <= 0x20 && !core_ok; ++ver) {
            NGXResult r = g_core_init_project(PROJECT_ID, 0, "0.1.0", g_work_dir.c_str(), g_device.Get(), ver, nullptr);
            core_ok = (r == NGX_SUCCESS);
        }
    }
    if (!core_ok) {
        for (int ver = 0x13; ver <= 0x20 && !core_ok; ++ver) {
            NGXResult r = g_core_init_ext(APP_ID, g_work_dir.c_str(), g_device.Get(), ver, &fci);
            core_ok = (r == NGX_SUCCESS);
        }
    }
    if (!core_ok) { SetError("NGX core initialization failed for API versions 0x13..0x20"); return false; }

    NGXResult sr = g_shim_init(reinterpret_cast<void*>(g_nr_init), APP_ID, g_work_dir.c_str(), g_device.Get(), 0x15, &fci);
    if (sr != NGX_SUCCESS) {
...
__declspec(dllexport) int __cdecl dlss5nr_init(int gpu_index, const wchar_t* runtime_dir, const wchar_t* caller_path, const wchar_t* work_dir, char* err, int err_cap) {
    std::lock_guard<std::mutex> guard(g_mutex);
    g_last_error.clear();
    if (g_initialized) { CopyError(err, err_cap); return 1; }
    if (!runtime_dir || !*runtime_dir || !caller_path || !*caller_path || !work_dir || !*work_dir) { SetError("runtime_dir is empty"); CopyError(err, err_cap); return 0; }

    g_gpu_index = gpu_index;
    g_runtime_dir = runtime_dir;
    g_caller_path = caller_path;
    g_work_dir = work_dir;
...
__declspec(dllexport) int __cdecl dlss5nr_process(
    ...
    // Output also supplies DLSSNR.Backbuffer for intensity compositing. Seed it
    // from this image on every call, including after feature/style/size changes;
    // otherwise low/zero intensity retains an earlier image or fresh black memory.
    auto prefill = Barrier(g_output.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_DEST);
    g_cmd->ResourceBarrier(1, &prefill);
    dst.pResource = g_output.Get();
    g_cmd->CopyTextureRegion(&dst, 0, 0, 0, &src, nullptr);
    auto prepared = Barrier(g_output.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    g_cmd->ResourceBarrier(1, &prepared);
