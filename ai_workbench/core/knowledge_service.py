...
class KnowledgeService:
    ...
    async def search(
        ...
        max_context_chars: int | None = None, include_debug: bool = False,
        ...
        return await search_knowledge(
            ...
            session_id=session_id, top_k=top_k, max_context_chars=max_context_chars,
            include_debug=include_debug, min_score_threshold=min_score_threshold,
    ...
    async def add_file(self, *, knowledge_base_id: str, path: str) -> Any:
        prepared=prepare_file_source(path=Path(path),root=self.repo_root); return await self.index_source(knowledge_base_id,prepared)

    async def reindex(self, source_id: str) -> Any:
    ...
    async def index_source(self, knowledge_base_id: str, prepared: Any, *, uri: str | None = None) -> Any:
        ...
        base = self.store.get_knowledge_base(knowledge_base_id)
        profile = self.model_manager.profile(base.embedding_model_profile_id, "embedding")
        validate_source_limits(prepared.text, prepared.size_bytes, settings)
        chunks = chunk_source_text(prepared.text, settings=settings, knowledge_base=base,
                                   source_title=prepared.title, source_uri=uri or prepared.uri)
        embedded = await self.model_manager.embed(profile.id, [item.content for item in chunks], purpose="document")
        ...
        return self.store.upsert_indexed_source(source=source,chunks=chunks,vectors=embedded.vectors,embedding_model_profile=profile,embedding_dimension=len(embedded.vectors[0]),search_texts=[build_search_text(prepared.title,item.heading_path,item.content,item.metadata) for item in chunks])
