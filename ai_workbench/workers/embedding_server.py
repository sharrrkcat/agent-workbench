"""Private token-authenticated Sentence Transformers worker, one profile per process."""
import os
...
if __package__:
    ...
    from .embedding_catalog import load_configuration
...
class EmbeddingWorker:
    ...
    def health(self):
        return {"protocol_version": 1, "loaded": [self.profile_id] if self.engine else [],
            ...
            "dimensions": self.engine.dimensions if self.engine else None}
    ...
    def dispatch(self, operation, body):
        ...
        try:
            if operation == "/load":
                ...
                if (body["kind"] != "embedding" or not isinstance(body["profile_id"], str)
                        or not 1 <= len(body["profile_id"]) <= 128):
                    raise WorkerError("INVALID_REQUEST")
                options, parameters = body["options"], body["parameters"]
                ...
                fields(parameters, ("query_prompt_name", "document_prompt_name"))
                if any(value is not None and (not isinstance(value, str) or not value) for value in parameters.values()):
                    raise WorkerError("INVALID_REQUEST")
                with stage("model_resources"):
                    path, information = load_configuration(self.root, body["model_ref"], parameters)
            ...
            if operation == "/unload":
                ...
                return self.health()
            if operation == "/embed":
                fields(body, ("profile_id", "input", "purpose", "dimensions"))
                strings(body["input"], limit=16)
                if body["purpose"] not in ("query", "document"):
                    raise WorkerError("INVALID_REQUEST")
                if body["dimensions"] is not None:
                    integer(body["dimensions"], 1, 65536)
                self.require_model(body["profile_id"])
                return self.engine.embed(body["input"], body["purpose"], body["dimensions"])
...
def main():
    ...
    with tracing(worker_trace(os.environ.get(TRACE_ENV), "worker_startup")):
        ...
        worker = EmbeddingWorker(Path(os.environ["COGITA_MODELS_ROOT"]).resolve())
