...
def libraries():
    with stage("engine_imports"):
        with stage("engine_imports.torch"):
            import torch
        with stage("engine_imports.transformers"):
            import transformers
        with stage("engine_imports.tokenizers"):
            import tokenizers
        with stage("engine_imports.PIL.Image"):
...
def load_processing(path, structure, config, transformers, tokenizers):
    ...
    with stage("processor_class_import"):
        processor_type = getattr(transformers, prefix + "ImageProcessorPil")
    with stage("image_processor"):
        processor = processor_type.from_dict(read_config(path / "preprocessor_config.json"))
    with stage("tokenizer_class_import"):
        tokenizer_type = transformers.PreTrainedTokenizerFast
        class SiglipTokenizer(tokenizer_type):
            @classmethod
            def convert_to_native_format(cls, trust_remote_code=False, **kwargs):
                # Transformers 5.16.1 otherwise loads a Rust object and then deep-copies it.
                return kwargs
    # Load the serialized backend directly: no SentencePiece conversion or remote code.
    with stage("tokenizer"):
        tokenizer = SiglipTokenizer.from_pretrained(str(path), tokenizer_file=str(path / "tokenizer.json"),
    ...
    with stage("processing_identity"):
        backend_signature = signature(json.loads(tokenizer.backend_tokenizer.to_str()))
    rules = {"pipeline": PIPELINE_VERSION, "structure": structure,
        ...
        "text": {"backend": backend_signature,
...
class SiglipEngine:
    def __init__(self, path, tower, options, revision):
        ...
        prefix = "Siglip2" if structure == "naflex" else "Siglip"
        with stage("config_class_import"):
            config_type = getattr(transformers, prefix + "Config")
        with stage("model_config"):
            config = config_type.from_dict(raw_config)
        ...
        with stage("model_from_pretrained"):
            self.model = model_type.from_pretrained(str(path), config=tower_config, local_files_only=True,
                trust_remote_code=False, use_safetensors=True, dtype=self.dtype)
        with stage("model_to_device"):
