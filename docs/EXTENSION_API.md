...
Index responses include `source_id`, `status`, `chunks`, `embedding_model_profile_id`, `embedding_dimension`, `indexed_at`, and `error`. Source and chunk inspection APIs return compact profile metadata only; they do not return full Markdown trees, full source originals, embedding vectors, or other large derived objects.
