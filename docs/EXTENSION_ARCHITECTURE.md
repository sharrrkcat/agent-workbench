...
- Intent Routing's long-term runtime pipeline is owned by core runtime: Semantic router -> Utility LLM slots/extraction -> Validator -> Executor. The semantic router remains the only classifier; Utility LLM is required for non-chat executable slots; validators and executors are the final safety boundary.
- Route candidates are derived from existing owners: General route examples, Knowledge Base names/aliases/descriptions, AgentConfig target hints, Agent action metadata, Capability command metadata, and compact Pet runtime list data. No Agent or Capability manifest route registry fields are added.
