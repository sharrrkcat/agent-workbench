...
- Managed origin directories are limited to safe relative folders under `data/knowledge/origins/<origin_folder>/`, including nested folders such as `starwars/canon`.
...
- `kb_origins` owns managed origin configuration, optional `default_chunk_profile`, and scan/import timestamps. `kb_sources` owns source metadata and local source references, including origin file relative paths, file status, and compact effective profile fields, not full pasted source text.
...
- Markdown source indexing supports `plain_text`, `markdown_document`, `markdown_collection`, and `markdown_auto`; profile precedence is frontmatter, source-create override for pasted text/text attachments, origin default, Knowledge Base default, Knowledge Defaults default, auto detector, then `markdown_document` fallback. Low-confidence auto detection falls back to `markdown_document`.
- Managed origin scan is lightweight metadata detection only; managed origin import/reindex is an explicit user action that runs the existing indexer. Rebuilding derived chunks, embeddings, and FTS rows must be explicitly triggered by the user. Arbitrary `local_file` sources, paths outside safe folders under `data/knowledge/origins/`, file watching, automatic sync, scheduled scans, background reindexing, chat-time scan/reindex, automatic model download, and retrieval ranking/backend changes remain out of scope.
...
- Knowledge Bases store per-KB configuration, chunk profile defaults/overrides, and Intent Routing aliases used only for natural-language KB hint matching.
...
- Knowledge Origins are Workbench-owned managed directory records under `data/knowledge/origins/<origin_folder>/`. The folder is a safe relative path under that root and may be nested. Creating an origin reuses an existing matching directory and rejects an existing non-directory path. Folder suggestion APIs list directory names only and do not scan or index. Origins never delete user source files. Scan marks source files new/changed/missing/failed/ready without changing derived index rows; Import/Reindex is the explicit operation that updates chunks, embeddings, and FTS rows for new or changed files. A user-triggered Create and index action may run one scan plus import/reindex after origin creation. Deleting an origin removes its source rows and derived chunks, embeddings, and FTS rows from the Knowledge Base but keeps disk files. Changing origin or KB chunk profile defaults marks affected indexed sources stale/reindex recommended but does not rebuild them.
