...
- Intent Routing semantic decision happens before `Building context`. The `Intent semantic routing` step shows a compact outcome such as `knowledge_query - executed`, `knowledge_query - not executed: Utility LLM unavailable`, `image_generation - not executed: diagnostic-only`, or `skipped: explicit command`. If auto mode authorizes a `knowledge_query` override, the temporary KB/query values are passed into that run's Knowledge context builder.
...
- `pet_command`: a narrow `/pet`-only allowlist for `/pet status`, `/pet wake`, `/pet tuck`, `/pet reload`, and `/pet select <pet_id>`. It reuses the normal Capability command runner and Pet runtime, records `target_command="/pet"` and one `generated_command`, and does not execute any other slash command or command sequence. Utility LLM slots must set `domain=workbench_pet` and an allowlisted action, then the Pet validator must pass. `/pet random` is not part of this allowlist.
...
`knowledge_query` auto execution requires all of the following: General mode `auto`, `intent_routing_auto_route_safe_intents=true`, eligible ordinary text in `single_assistant` mode, current default Agent is an effective Intent Routing enabled Prompt Agent, semantic predicted intent is `knowledge_query`, grouped intent score and margin meet the semantic intent thresholds, Utility LLM is available, Utility LLM slots extraction succeeds, `slots.intent=knowledge_query`, and either `slots.query` is non-empty or `slots.use_original_query=true`. The Knowledge validator selects a KB from an exact Utility `kb_hint` match, a semantic KB candidate above `intent_routing_semantic_kb_min_score`, or active session KB bindings when no explicit KB candidate exists. Utility LLM slots may come from a local `transformers`/`llama_cpp` backend or from the configured `model_profile` backend. Utility LLM unavailable records `not_executed_reason=utility_llm_required` or `utility_llm_unavailable`; slots failure records `utility_slots_failed`; invalid JSON records `utility_invalid_json`; missing query records `knowledge_query_missing_query`; KB conflicts/ambiguity record `kb_hint_semantic_conflict` or `ambiguous_kb_candidate`; no candidate/bindings records `no_kb_candidate_or_active_kbs`. No local query regex/heuristic extractor may replace Utility slots. The execution keeps the current Prompt Agent, sets only per-run `temporary_knowledge_base_ids` and `knowledge_query_override`, and never persists Context Sources bindings or changes the session default Agent.

`pet_command` auto execution requires the same General/session/Prompt-Agent eligibility gates as other safe auto routes, semantic predicted intent `pet_command`, semantic score at or above the intent minimum, Utility LLM availability, Utility LLM slots extraction success, `slots.intent=pet_command`, `slots.domain=workbench_pet`, and `slots.action` in `status | wake | tuck | select | reload`. Regex or keyword helpers may only provide exact matching/normalization hints; they must not override semantic score/margin or independently determine action/target execution. Low semantic margin records `semantic_margin_too_low` as a warning for `pet_command` but does not block execution after Utility slots and validation pass. The validator resolves target candidates from the existing Pet Capability runtime/settings/list data only when the action needs a target. `status` always generates `/pet status`. `wake` without a target generates `/pet wake`; `wake` with a unique target generates `/pet select <pet_id>` because selecting activates and wakes that pet. `tuck` always generates `/pet tuck`, and `reload` always generates `/pet reload`; any target hint is ignored and may record `pet_target_ignored_for_action`. `select` requires a unique target and otherwise records `select_target_missing`, `pet_candidate_not_found`, or `ambiguous_pet_candidate`. `source_pet_hint` is diagnostic metadata only and never blocks execution. Reality-pet questions such as sick cats/dogs, training real pets, or fictional character questions without Workbench Pet context must not execute and should record `not_workbench_pet_context` when Utility identifies a non-Workbench domain.
...
`pet_command` metadata may include compact Pet slots: `pet_action`, `target_pet_id`, `target_pet_hint`, `source_pet_id`, `source_pet_hint`, `target_ignored_for_action`, `generated_command`, `route_action="pet_command"`, `auto_executable`, `executed`, `not_executed_reason`, and `warnings`. It must not store full Pet manifests, spritesheet content, image bytes, or large Pet objects.
...
The Utility LLM `model_profile` backend does not create an Agent run or visible message, does not recursively trigger Intent Routing or title generation, does not inject Knowledge/Core Memory/Worldbook/attachments/history, does not mutate the session selected Model Profile, and does not change the main response LLM resolution. Runtime metadata may record public profile/provider/model identifiers only.
...
- `knowledge_query` and `pet_command` require Utility LLM availability and strict JSON slot success before auto execution. Utility unavailable records `utility_llm_required` or `utility_llm_unavailable`; extraction failure records `utility_slots_failed`; invalid JSON records `utility_invalid_json`.
- `knowledge_query` slots include at least `intent`, `query`, `kb_hint`, and optional `use_original_query`. The validator checks enabled KB candidates, semantic/slot conflicts, ambiguity, non-empty query, and active KB availability without mutating session bindings.
- `pet_command` slots include at least `intent`, `domain`, `action`, `target_pet_hint`, and `source_pet_hint`, plus optional `target_pet_explicit` and `source_pet_explicit`. The validator requires `domain=workbench_pet`, an allowlisted action, and unique target resolution only for `wake` with target and `select`. It does not perform current-pet checks for wake/tuck/reload and does not require source-pet consistency.
...
For `pet_command`, Route Test also reports Pet action, Utility slots, resolved target Pet when relevant, whether a target was ignored for the action, validation result, single generated command, would-execute status, not-executed reason, and warnings. Route Test never executes `/pet`, never creates a command run, and never updates Pet settings.

Common `not_executed_reason` and warning codes are stable lowercase strings. UI clients should map them to localized labels instead of displaying raw codes by default. Common codes include `semantic_router_unavailable`, `semantic_router_profile_missing`, `semantic_router_embedding_unavailable`, `semantic_confidence_too_low`, `semantic_margin_too_low`, `utility_llm_required`, `utility_llm_unavailable`, `utility_slots_failed`, `utility_invalid_json`, `utility_semantic_intent_conflict`, `utility_semantic_action_conflict`, `validation_failed`, `knowledge_query_missing_query`, `kb_hint_semantic_conflict`, `ambiguous_kb_candidate`, `no_kb_candidate_or_active_kbs`, `pet_candidate_not_found`, `ambiguous_pet_candidate`, `select_target_missing`, `pet_target_ignored_for_action`, `not_workbench_pet_context`, `diagnostic_only`, `image_generation_action_routing_not_ready`, `command_like_auto_route_disabled`, `agent_route_auto_route_disabled`, `action_route_auto_route_disabled`, `compound_intent_not_auto_routed`, `explicit_command`, `explicit_agent`, `explicit_action`, and `group_transcript_not_supported`.
...
- Utility LLM does not participate in main LLM resolution. It is configured by General settings and displayed under Settings -> General -> Utility LLM, can use `transformers`, `llama_cpp`, or an LLM `model_profile` backend, is not itself a Provider Profile or Model Profile, and does not change the user's selected main model. The `model_profile` backend is a direct internal non-streaming short call with deterministic defaults and no conversation history.
...
Backend resolution:
- `utility_llm`: uses the configured Utility LLM first, including its selected `model_profile` backend when configured. If it is unconfigured, unavailable, missing dependencies/model files/profile/provider, fails generation, or returns an invalid title, resolution falls back to `follow_agent_model_profile`.
...
- Utility LLM title generation calls the Utility LLM unload helper when release is requested for local Utility backends and records `released`, `no_supported_release`, or `failed`. When Utility LLM uses `model_profile`, the Utility LLM unload endpoint is not used for global LLM release; supported title model release follows the existing provider unload helper and must not unload the current response model.
...
Source indexing supports `pasted_text`, text attachment sources, and explicit `origin_file` imports from managed origins under `data/knowledge/origins/<origin_folder>/`. The origin folder is a safe relative path under that managed root and may be nested, but it cannot be absolute, contain traversal, empty segments, or backslashes. Pasted source originals are stored as text files under `data/knowledge/sources`; full source originals are not stored in SQLite. Pasted/text attachment source-create requests may include a virtual `folder_path` for Source list grouping and a `chunk_profile` source override. These are Knowledge Base virtual paths, not arbitrary server filesystem paths. The indexer chunks source text, embeds chunks with the Knowledge Base embedding profile using `purpose=document`, stores float32 vectors in SQLite BLOB rows, and writes FTS5/BM25-ready rows.

Managed origin scan and import are separate runtime operations. Scan only validates the managed origin path, walks supported text files, compares mtime/size/hash, and records source file status such as new, changed, missing, failed, or ready. Scan does not parse, chunk, embed, write chunks, write embeddings, write FTS rows, delete old derived indexes, or change retrieval-visible indexes. Import/Reindex is the explicit operation that sends new and changed origin files through the normal indexer. Settings may offer a user-triggered Create and index action that runs one scan plus import/reindex after origin creation. Missing files keep old derived indexes until the user deletes the source or deletes the origin. Deleting an origin removes its source rows and derived chunks/embeddings/FTS rows from the Knowledge Base but never deletes files from disk. Stale status is a source-management/UI hint; retrieval and context injection keep using existing ready index rows. Chat, Prompt Agent Knowledge injection, Script Agent `ctx.llm` Knowledge injection, `/kb-search`, query expansion, embeddings, and reranking never trigger scan, import, cleanup, or reindex.

Markdown source indexing supports chunk profiles: `plain_text`, `markdown_document`, `markdown_collection`, and `markdown_auto`. Resolution precedence is frontmatter `chunk_profile`, source-create override for pasted text/text attachments, origin default, Knowledge Base default, Knowledge Defaults default, markdown auto detection, then `markdown_document` fallback. Changing an origin, KB, or Knowledge default profile marks affected sources as `needs_reindex` without changing existing chunks or embeddings. The deterministic Markdown parser reads simple frontmatter, ATX headings `#` through `######`, and ignores headings inside fenced code blocks. It records compact per-chunk metadata in `kb_chunks.metadata_json`: `chunk_title`, `document_title`, `entity_type`, `heading_path`, `line_start`, `line_end`, `char_start`, `char_end`, `chunk_profile_requested`, `chunk_profile_effective`, `chunk_profile_confidence`, `profile_source`, `entity_level`, `title_source`, and `type_source`.

`chunk_title` is the retrieval-facing title used in the embedding input `Title:` line. It is not Semantic Router metadata and is not session title generation metadata. `markdown_document` uses the document title, from frontmatter `title`, H1, or titleized filename, as every chunk's `chunk_title`; lower headings only shape `heading_path`. `markdown_collection` uses entity headings as `chunk_title` and maps parent category headings such as `Characters`, `Locations`, and `Factions` to compact `entity_type` values. `markdown_auto` scores document-like and collection-like structure without an LLM. Low confidence or ambiguous results fall back to `markdown_document`. Origin files preserve relative path metadata in source records and chunk metadata, and the embedding document input includes that path in its `Path:` line.
