...
Managed origin scan and import are separate runtime operations. Scan only validates the managed origin path, walks supported text files, compares mtime/size/hash, and records source file status such as new, changed, missing, failed, or ready. Scan does not parse, chunk, embed, write chunks, write embeddings, write FTS rows, delete old derived indexes, or change retrieval-visible indexes. Import/Reindex is the explicit operation that sends new and changed origin files through the normal indexer. Missing files keep old derived indexes until the user deletes the source. Chat, Prompt Agent Knowledge injection, Script Agent `ctx.llm` Knowledge injection, `/kb-search`, query expansion, embeddings, and reranking never trigger scan or import.

Markdown source indexing supports chunk profiles: `plain_text`, `markdown_document`, `markdown_collection`, and `markdown_auto`. Markdown defaults to `markdown_auto`; non-Markdown text keeps the `plain_text` character-window behavior. A Markdown source may override auto detection with frontmatter `chunk_profile: markdown_document`, `chunk_profile: markdown_collection`, or `chunk_profile: plain_text`. The deterministic Markdown parser reads simple frontmatter, ATX headings `#` through `######`, and ignores headings inside fenced code blocks. It records compact per-chunk metadata in `kb_chunks.metadata_json`: `chunk_title`, `document_title`, `entity_type`, `heading_path`, `line_start`, `line_end`, `char_start`, `char_end`, `chunk_profile_requested`, `chunk_profile_effective`, `chunk_profile_confidence`, `title_source`, and `type_source`.
