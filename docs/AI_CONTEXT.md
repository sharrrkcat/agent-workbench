...
- Semantic auto execution is limited to `chat`, high-confidence `knowledge_query`, and the narrow `pet_command` `/pet` allowlist. `chat` keeps the current Prompt Agent path and adds no temporary Knowledge override. Low semantic margin is a warning, not a hard block, for `pet_command` when semantic score, Utility slots, domain/action allowlist, and validator checks pass.
- Intent Routing v2 pipeline is Semantic router -> Utility LLM slots/extraction -> Validator -> Executor. `chat` is semantic-only. Non-chat executable intents such as `knowledge_query` and `pet_command` must pass Utility LLM strict JSON slots and validator checks before execution.
- Do not use regex, keywords, or heuristic parsers as the primary replacement for Utility LLM natural-language slots. Pet regex work is allowed only as explicit syntax parsing, slot hints, false-positive guards, exact matching support, or validator input; do not keep expanding pet regex as the main parser.
- Utility LLM may use a local backend or an existing LLM Model Profile backend. Backend/model settings belong under General -> Utility LLM. Do not register Utility LLM as a Model Profile, Provider Profile, Agent, Capability, or slash command.
...
- `pet_command` auto routing may execute only one `/pet status`, `/pet wake`, `/pet tuck`, `/pet reload`, or `/pet select <pet_id>` through the normal CommandRunner and Pet Capability runtime. `/pet wake`, `/pet tuck`, and `/pet reload` do not accept pet parameters; waking a named pet maps to `/pet select <pet_id>`. It must not execute other slash commands, build multi-command sequences, directly mutate Pet settings outside `/pet`, store Pet manifests or image data in metadata, or implement `/pet random`.
...
- Utility LLM may support title generation and shadow JSON extraction, including through a selected LLM Model Profile backend, but it must not be a Model Profile, Provider Profile, Capability, Agent, or slash command. Do not use the Model Profile backend to inject Memory, Worldbook, Knowledge, attachments, or chat history.
...
- For new Intent Routing intents or actions, prefer adding a RouteSpec/ActionSpec, SlotSchema, validator binding, and executor binding. Do not add broad regex route parsers as the primary routing mechanism. Do not modify Agent or Capability manifests for route candidate work. Route Test and real runs must share validator/executor planning or the same gate path. Start from this AI_CONTEXT section and targeted grep before reading large source lists.
- After Intent Routing v2 Round 4, `knowledge_query` and `pet_command` are migrated to the shared RouteSpec/ActionSpec -> Utility slot schema -> Validator -> ExecutorPlan path. Do not reintroduce adapter-based Knowledge overrides, regex-driven Pet command execution, or Route Test logic that bypasses the shared validator/planner.
- After Intent Routing v2 Round 5, Route Test and real runs must keep using the same gating result. Route Test may show Summary plus folded Diagnostics, but it must not force `would_execute=true` merely because a route is auto-executable, and it must never execute commands, Knowledge retrieval, ComfyUI, Agents, or session mutations.
- Keep `metadata.intent_routing` compact. Do not store raw Utility LLM output, Utility prompts, embeddings, full examples/specs/candidates, Agent prompts, KB content, Worldbook/Core Memory content, full chat history, Pet manifests, spritesheets, or duplicate full original user text.
...
- Managed Knowledge origins are manual-only directories under safe relative folders in `data/knowledge/origins/<origin_folder>/`, including nested folders when explicitly created or selected; scan detects file status only, while import/reindex performs indexing. Folder suggestion APIs list directory names only and do not scan or index. Deleting an origin removes its source rows and derived chunks/embeddings/FTS rows from the Knowledge Base but never deletes files from disk. Do not add automatic sync, file watching, scheduled scans, or chat-time reindex behavior.
- Chunk profile work must preserve `chunk_title` semantics and avoid adding `semantic_title`. Profile precedence is frontmatter, source-create override for pasted text/text attachments, origin default, Knowledge Base default, Knowledge Defaults default, auto detector, then `markdown_document` fallback. Changing profile defaults should mark affected sources stale/reindex recommended only; do not rebuild automatically.
