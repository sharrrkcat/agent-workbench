...
- `image_generation` is paused as diagnostic-only in semantic auto routing until action routing is designed. Do not route image-generation predictions to `comfyui_agent` from semantic or rule-based fallback decisions in this round.
...
- Route Test and real chat run gating logic must stay aligned. Do not let Route Test report execution for a semantic decision that the runtime auto gate would reject, and do not let the runtime use legacy high/low confidence thresholds as the primary semantic auto-route gate.
