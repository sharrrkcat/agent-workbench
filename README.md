...
Chunk profile resolution is deterministic and never uses an LLM. The precedence is: source frontmatter `chunk_profile`, source override when supported by a future source-edit contract, origin `default_chunk_profile`, Knowledge Base `default_chunk_profile`, Knowledge Defaults `default_chunk_profile`, markdown auto detection, then `markdown_document` fallback. Current UI/API support frontmatter, origin default, Knowledge Base default, and Knowledge Defaults; source-level override is reserved. Changing an origin/KB/default profile marks affected indexed sources as `needs_reindex` and leaves existing chunks, embeddings, and FTS rows untouched until the user explicitly imports or reindexes.
...
Managed origins are manual directory imports only. Users may place supported text files under `data/knowledge/origins/<origin_slug>/` and create the matching origin in Settings -> Knowledge -> Knowledge Bases. `Scan now` only walks that managed directory, validates paths, checks size/type/hash/mtime, and marks sources as new, changed, missing, failed, or ready. It does not parse Markdown, chunk text, embed vectors, write FTS rows, delete source indexes, or change retrieval-visible indexes. `Import new files`, `Reindex changed files`, `Reindex source`, and `Import / Reindex` are explicit actions that run the normal indexing path. Missing files keep their old chunks/embeddings/FTS rows until the user deletes the source through the existing delete flow. Chat/Prompt Agent Knowledge use reads the existing ready index only; file changes only become retrieval-visible after explicit reindex. There is no automatic sync, file watcher, scheduled scan, background rebuild, or chat-time scan/reindex.
