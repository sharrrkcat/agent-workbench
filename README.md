...
Web Context can also optionally use the Utility LLM to judge whether filtered search candidates are relevant evidence for the current question before page fetching. This best-effort judge uses strict JSON, stores only compact diagnostics, and falls back to normal filtered results if the Utility LLM is unavailable or returns invalid JSON.
