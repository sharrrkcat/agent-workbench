...
Settings -> General -> Web Search controls whether ordinary Prompt Agent messages inject `# Retrieved Web` during context building. The Chat composer Web Search toggle updates that same global setting. With Intent Routing off or in shadow mode, eligible ordinary Prompt Agent messages search the current user input through the Web Search Capability's SearXNG helper. With Intent Routing auto mode enabled, the runtime first builds a Web Context plan: selected Knowledge and Pet routes skip Web Context, validated `web_query` slots may provide a compact query, and uncertain chat/diagnostic outcomes use a short Utility LLM JSON check to decide whether the user is actually asking for external facts. Settings -> Capabilities -> Web Search still owns the SearXNG provider connection, domain filters, result de-duplication, diagnostics, and `/web-search` command configuration.
Web Context can optionally fetch the top filtered result pages as compact plain-text excerpts for Prompt Agent context only. Page fetching is best-effort, supports HTTP/HTTPS HTML without JavaScript or browser rendering, and does not affect `/web-search`, Settings test search, Knowledge storage, or vector indexes.
Web Context can also optionally use the Utility LLM as a conservative reject-only noise filter before page fetching. It may remove clearly unhelpful candidates, but uncertain or unjudged candidates are kept by default; the Utility LLM does not choose the final source count or independently decide all Web sources. This best-effort judge uses strict JSON, stores only compact diagnostics, and falls back to normal filtered results if the Utility LLM is unavailable or returns invalid JSON.
