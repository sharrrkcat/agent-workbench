...
Settings -> General -> Utility LLM configures the optional Utility LLM core service for short internal tasks: automatic session title generation and shadow-mode intent JSON extraction/verification. The Utility LLM can use either a `transformers` / Hugging Face model folder such as `data/models/utility_llms/Qwen3-0.6B`, selected by `utility_llms/Qwen3-0.6B`, or a GGUF file through the optional `llama_cpp` backend. GGUF files must live under an individual model folder, for example `data/models/utility_llms/qwen3-0.6b/Qwen3-0.6B-Q4_K_M.gguf`, selected by `utility_llms/qwen3-0.6b/Qwen3-0.6B-Q4_K_M.gguf`. Root-level GGUF files directly under `data/models/utility_llms` are not scanned or accepted. The Utility LLM is not a Model Profile, Provider Profile, Agent, Capability, or slash command. The app never downloads the model or installs optional dependencies; if `transformers`/`torch`, `llama-cpp-python`, the configured model folder/file, or generation fails, normal chat startup still works and title generation can fall back to the configured title Model Profile path.
...
Settings -> General stores local app settings in SQLite. The General page is split into `Files`, `LLM & Prompts`, `Memory`, `Utility LLM`, and `Intent Routing` categories. `Files` contains upload and text-file context limits. `LLM & Prompts` contains automatic session title generation, title backend selection, optional specific title Model Profile, best-effort title model release, the title prompt and input limit, plus context prompt overrides. `Memory` stores manually maintained Core Memory text and separate Prompt Agent / Script Agent enablement flags. `Utility LLM` contains the internal small-model backend, model path, scan, runtime options, status, test, and unload controls. `Intent Routing` contains route behavior, examples, route test, the semantic router Embedding Model Profile selector/status, candidate counts, and a compact Utility LLM status summary. Prompt Agents inject non-empty Core Memory by default as system context. Script Agent `ctx.llm.*` calls inject Core Memory only when the Script Agent switch is enabled.
...
- Utility LLM settings, including backend, model path, device, and llama.cpp options
