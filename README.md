...
- `pet_command` may execute only the existing `/pet` Capability command path for status, wake, tuck, select, or reload after Utility slots set `domain=workbench_pet`, an allowlisted action, and the Pet validator passes. Regex and deterministic helpers are limited to hints/validation and cannot replace Utility slots. `/pet random` is not implemented.
