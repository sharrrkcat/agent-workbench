...
When a session still has a default title such as `Session 1`, the core runtime can make one best-effort attempt to generate a short title after the first user message that resolves to an LLM-capable Agent/action. Non-LLM slash commands such as memory, pet, or Knowledge search commands do not trigger title generation. Explicit `@agent`, `@agent:action`, `:action`, Script Agent LLM calls, and safe Intent Routing auto routes can trigger titles when the final invoked Agent/action uses an LLM. The title prompt uses only the triggering user message, not assistant replies, slash command results, prior history, Knowledge snippets, Worldbooks, or Core Memory. Long user input is truncated from the middle with head/tail preservation according to the General title input limit.

Settings -> General -> LLM & Prompts -> Session Titles supports three title backends. `Utility LLM` is the default and uses the small local Utility LLM first; if it is unavailable or fails validation, it falls back to `Follow Agent model profile`. `Follow Agent model profile` resolves the composer/session input override first, then the session default Agent model profile, then the actual invoked Agent model profile. `Specific model profile` uses only the configured title Model Profile and skips title generation with a warning if that profile is missing, disabled, or unavailable. Title generation is internal, non-streaming, creates no chat messages, does not trigger Intent Routing, and does not change the selected Agent or Model Profile. The optional `Free title model after generation` setting is best-effort; if the title model is also used by the current response, release is deferred until the run finishes. If title generation fails, the original title is kept and the main conversation continues. There is no title history or regenerate button in this alpha.
...
Settings -> General -> Intent Routing also supports custom route examples for the built-in intents `chat`, `image_generation`, `knowledge_query`, `agent_route`, and `command_like`. Examples are one per line and are merged with built-in examples in the semantic route index; they help recognize similar user messages but do not execute commands or expand the safe auto-route allowlist. The same panel includes a Route Test tool that predicts a message without creating a chat message or run, without calling ComfyUI, and without running Knowledge retrieval. Intent Routing shows only a compact Utility LLM status summary; detailed Utility LLM configuration lives in Settings -> General -> Utility LLM.

The semantic router uses the selected Knowledge Embedding Model Profile from `intent_routing_embedding_model_profile_id`. Candidate documents are embedded with `purpose=document`; the current user input is embedded with `purpose=query`. The lazy in-memory route index contains built-in RouteSpec/ActionSpec examples, General custom route examples, enabled Knowledge Base names/aliases/descriptions, Agent names/routing aliases/examples, and weak diagnostic candidates from existing Agent actions and Capability commands. RouteSpec and ActionSpec are core runtime internals used as semantic candidates and Utility LLM slot schema sources; they are not new user-editable Agent or Capability manifest schema fields. The embedding semantic router is the only Intent Routing classifier. Intent Routing v2 uses a fixed pipeline: Semantic router -> Utility LLM slots/extraction -> Validator -> Executor. `chat` is the semantic-only exception and keeps the current Prompt Agent path. `knowledge_query` and `pet_command` now use the shared RouteSpec pipeline instead of adapter or regex-driven main execution paths. In auto mode, only `chat`, Utility-approved `knowledge_query`, and Utility-approved narrow `pet_command` can execute. General command-like requests, generic Agent routes, Agent actions, image generation, and compound/multi-intent matches are metadata only in this version.

Settings -> General -> Utility LLM configures the optional Utility LLM core service for short internal tasks: automatic session title generation and shadow-mode intent JSON extraction/verification. The Utility LLM can use three backends: a `transformers` / Hugging Face model folder such as `data/models/utility_llms/Qwen3-0.6B`, selected by `utility_llms/Qwen3-0.6B`; a GGUF file through the optional `llama_cpp` backend; or an existing LLM Model Profile. GGUF files must live under an individual model folder, for example `data/models/utility_llms/qwen3-0.6b/Qwen3-0.6B-Q4_K_M.gguf`, selected by `utility_llms/qwen3-0.6b/Qwen3-0.6B-Q4_K_M.gguf`. Root-level GGUF files directly under `data/models/utility_llms` are not scanned or accepted. The Model Profile backend sends only the current input plus compact route/title context to the selected provider; it does not send KB content, Worldbook content, Core Memory, Agent prompts, or full chat history, and it does not change the main chat Model Profile selection. The Utility LLM is not itself a Model Profile, Provider Profile, Agent, Capability, or slash command. The app never downloads the model or installs optional dependencies; if `transformers`/`torch`, `llama-cpp-python`, the configured local model, selected Model Profile, or generation fails, normal chat startup still works and title generation can fall back to the configured title Model Profile path.

For GGUF support, install `llama-cpp-python` yourself in the backend environment if needed, for example with `uv pip install llama-cpp-python`. CUDA, Metal, and other accelerated builds vary by system, so follow the llama-cpp-python documentation for those builds. Use Settings -> General -> Utility LLM to scan local Utility LLM models and run the title generation / JSON extraction tests to verify a configured backend.

The Utility LLM JSON extractor provides required strict JSON slots for non-chat executable intents such as `knowledge_query` and `pet_command`. It is not the primary classifier and cannot execute directly; validator approval is required before executor planning. Its prompt receives only the current text, semantic intent/action candidate, compact top RouteSpec/ActionSpec data, compact slot schemas, compact Knowledge Base names/aliases, compact Pet candidates for `pet_command`, and safety boundaries. It does not receive Agent prompts, KB content, Worldbook content, Core Memory content, full history, full candidates, or embeddings. A Model Profile Utility backend uses the same compact prompt through an internal non-streaming call and records only public profile/provider/model identifiers. In `shadow` mode it still never changes routing. Auto execution remains deliberately narrow:

- `chat` keeps the current Prompt Agent path and does not add a temporary Knowledge override.
- `knowledge_query` keeps the current Prompt Agent path but may provide a per-run temporary Knowledge Base/query override only when Utility slots and the Knowledge validator pass. This does not persist Context Sources bindings and does not change retrieval ranking.
- `pet_command` may execute only the existing `/pet` Capability command path for `/pet status`, `/pet wake`, `/pet tuck`, `/pet reload`, or `/pet select <pet_id>` after Utility slots set `domain=workbench_pet`, an allowlisted action, and the Pet validator passes. `/pet wake`, `/pet tuck`, and `/pet reload` do not accept pet parameters. Natural-language requests to wake a specific pet map to `/pet select <pet_id>` because selecting a pet activates and wakes it. Source pet hints are diagnostic only, and target hints are ignored for status/tuck/reload. Regex and deterministic helpers are limited to hints/validation and cannot replace Utility slots. `/pet random` is not implemented.
- `image_generation` is diagnostic-only and does not route to ComfyUI until action routing is designed.
- `command_like`, generic `agent_route`, `action_route`, and `compound` remain diagnostic-only and are not executed.

Route Test reports the full pipeline: semantic prediction, RouteSpec/ActionSpec ids, slot schema id, Utility LLM required/available/used/slots state, validator result, and a single-command execution plan including `would_execute`, `route_action`, `generated_command`, warnings, and temporary Knowledge override preview. It is diagnostic-only: it does not create messages or runs, execute `/pet`, run Knowledge retrieval, call ComfyUI, or mutate session settings. Utility LLM unavailable means non-chat auto execution is unavailable and `not_executed_reason` is `utility_llm_required` or `utility_llm_unavailable`; slot extraction failure uses `utility_slots_failed`, and invalid JSON uses `utility_invalid_json`. Explicit `/command`, `@agent`, `@agent:action`, and `:action` inputs always bypass Intent Routing, so explicit `/pet wake` remains a normal slash command. Natural-language `pet_command` auto routing preserves the original visible user message and records the generated `/pet ...` command only in metadata. Low semantic margin records a warning for `pet_command` but does not block execution when the score, Utility slots, domain, action allowlist, and Pet validator pass. `command_like` intents are recorded but are not executed automatically, so requests such as freeing memory do not run `/free-memory` in this version. Auto routing does not change the session default Agent, the visible Agent selector, Pet settings except through the executed `/pet` command itself, or Context Sources Knowledge bindings. There is no raw embedding path setting; the semantic router only accepts an existing enabled Knowledge Embedding Model Profile and never downloads models automatically.

Knowledge Bases can store comma-separated aliases such as `星战, Star Wars, SW`; Intent Routing uses enabled KB names, aliases, and descriptions as semantic candidates for `knowledge_query` metadata. Agents can store routing aliases and route examples under Agent detail -> Intent Routing as runtime target hints. Prompt Agents also expose their per-Agent entry override in that tab. These values remain `AgentConfig.runtime` fields and are not written to `agent.yaml`; generic Agent routes still require a future confirmation flow and are not executed automatically.
...
- managed origin directories under safe relative folders in `data/knowledge/origins/<origin_folder>/`.
...
- Markdown chunk profiles for source indexing: `plain_text`, `markdown_document`, `markdown_collection`, and `markdown_auto`.
...
Chunk profile resolution is deterministic and never uses an LLM. The precedence is: source frontmatter `chunk_profile`, source override from source-create/import UI when provided, origin `default_chunk_profile`, Knowledge Base `default_chunk_profile`, Knowledge Defaults `default_chunk_profile`, markdown auto detection, then `markdown_document` fallback. Current UI/API support frontmatter, source-create override for pasted text/text attachments, origin default, Knowledge Base default, and Knowledge Defaults. Changing an origin/KB/default profile marks affected indexed sources as `needs_reindex` and leaves existing chunks, embeddings, and FTS rows untouched until the user explicitly imports or reindexes.

Chunk metadata is stored compactly on `kb_chunks.metadata_json` with fields such as `chunk_title`, `document_title`, `entity_type`, `heading_path`, source path, source line/character offsets, `chunk_profile_requested`, `chunk_profile_effective`, `chunk_profile_confidence`, `profile_source`, `entity_level`, and title/type sources. Source API responses expose the same compact effective profile fields for Settings display. `chunk_title` is the RAG retrieval-facing title used in the embedding `Title:` line; it is not Semantic Router metadata and is not automatic session title generation metadata.

Managed origins are manual directory imports only. Users may place supported text files under `data/knowledge/origins/<origin_folder>/` and create the matching origin in Settings -> Knowledge -> Knowledge Bases. Origin folders are safe relative paths under that root and may be nested, such as `starwars/canon`; absolute paths, traversal, empty segments, and backslashes are rejected. Existing directories under that managed root are reused when the origin folder matches; an existing non-directory path is rejected. Folder suggestions list existing directories only and do not scan or index. `Scan` only walks that managed directory, validates paths, checks size/type/hash/mtime, and marks sources as new, changed, missing, failed, or ready. It does not parse Markdown, chunk text, embed vectors, write FTS rows, delete source indexes, or change retrieval-visible indexes. `Reindex source`, Knowledge Base `Reindex all`, origin `Reindex`, and the explicit Create and index action run the normal indexing path. Deleting an origin removes its source rows and derived chunks/embeddings/FTS rows from the Knowledge Base but does not delete the origin folder or files on disk. Missing files keep their old chunks/embeddings/FTS rows until the user deletes the source or deletes the origin. Chat/Prompt Agent Knowledge use reads the existing ready index only; file changes only become retrieval-visible after explicit reindex. There is no automatic sync, file watcher, scheduled scan, background rebuild, or chat-time scan/reindex.
...
Settings -> General stores local app settings in SQLite. The General page is split into `Files`, `LLM & Prompts`, `Memory`, `Utility LLM`, and `Intent Routing` categories. `Files` contains upload and text-file context limits. `LLM & Prompts` contains automatic session title generation, title backend selection, optional specific title Model Profile, best-effort title model release, the title prompt and input limit, plus context prompt overrides. `Memory` stores manually maintained Core Memory text and separate Prompt Agent / Script Agent enablement flags. `Utility LLM` contains the internal utility backend selection, local model path or Model Profile reference, scan, runtime options, status, test, and unload controls. `Intent Routing` contains route behavior, examples, route test, the semantic router Embedding Model Profile selector/status, candidate counts, and a compact Utility LLM status summary. Prompt Agents inject non-empty Core Memory by default as system context. Script Agent `ctx.llm.*` calls inject Core Memory only when the Script Agent switch is enabled.
...
- session title generation backend
- optional specific session title Model Profile id
- whether to best-effort unload the title model after generation
...
- Utility LLM settings, including backend, optional Model Profile id, local model path, device, and llama.cpp options
- Intent Routing settings, including optional embedding model profile id, semantic auto-route thresholds, and per-intent custom route examples

Use `GET /api/settings/general` and `PATCH /api/settings/general` to read and update these values. Unknown fields are rejected, except old stored raw Intent Routing embedding path keys are ignored for compatibility. Empty title prompts are rejected, and upload limits are enforced by the backend. File context settings only affect ordinary text/code/config files; image Vision input is still controlled by the selected model profile capability flags.
...
The `pet` Capability exposes `/pet`, `/pet status`, `/pet wake`, `/pet tuck`, `/pet reload`, and `/pet select <pet_id>`. `wake`, `tuck`, and `reload` act on the current default pet and do not accept pet parameters; `select <pet_id>` switches to and wakes that pet. These commands update Pet settings directly and do not call an LLM or create a Pet Agent. The overlay is an in-app chat overlay only; there is no desktop-level overlay or random movement.
