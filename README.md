...
Settings -> General -> Intent Routing adds an optional natural-language pre-route layer for ordinary messages. Explicit `/command`, `@agent`, `@agent:action`, and `:action` syntax always bypasses it. Shadow mode records compact predictions only; safe auto mode can execute only the documented `chat`, high-confidence Knowledge query override, and narrow Workbench Pet command paths. `web_query` can identify current/web-search requests and provide compact query signals to Prompt Agent Web Context when Web Search is enabled; Intent Routing does not directly execute `/web-search` or Web Search Capability commands.

Settings -> General -> Web Search controls whether ordinary Prompt Agent messages inject `# Retrieved Web` during context building. The Chat composer Web Search toggle updates that same global setting. With Intent Routing off or in shadow mode, eligible ordinary Prompt Agent messages search the current user input through the Web Search Capability's SearXNG helper. With Intent Routing auto mode enabled, the runtime first builds a Web Context plan: selected Knowledge and Pet routes skip Web Context, validated `web_query` slots may provide a compact query, and uncertain chat/diagnostic outcomes use a short Utility LLM JSON check to decide whether the user is actually asking for external facts. Web Search also supports customizable internal prompt templates for the plan resolver, candidate judge, page excerpt gate, and final Web Context injection; current local/UTC time is injected automatically for freshness-sensitive judgments. Settings -> Capabilities -> Web Search still owns the SearXNG provider connection, domain filters, result de-duplication, diagnostics, and `/web-search` command configuration.
Web Context can optionally fetch retained result pages progressively as compact plain-text excerpts for Prompt Agent context only. Page fetching includes deterministic HTML content cleaning before excerpt gating or injection, so common navigation, footer, ads, repeated lines, and link-heavy boilerplate are reduced before the main model sees page excerpts. Page fetching is best-effort, supports HTTP/HTTPS HTML without JavaScript or browser rendering, and does not affect `/web-search`, Settings test search, the HTTP Capability, Knowledge storage, or vector indexes.
Web Context can also optionally use a conservative Utility LLM candidate noise filter before page fetching, and a Page Excerpt Gate before injecting fetched page excerpts. The gate can follow the current Prompt Agent model, use a specific Model Profile, or use the Utility LLM. It performs an internal non-streaming JSON judgment, stores only compact diagnostics, and if unavailable or invalid rejects only that excerpt while the main run continues.
Web Search responses can cite injected sources with `[W1]`-style markers. The chat renderer turns known `[W#]` tokens into clickable source citations, and the Injected context modal can show compact Web source details alongside Knowledge snippets and Worldbook entries.
...
Provider Profiles hold model source details such as provider label, base URL, API key, timeout, enabled state, and provider-specific metadata. They can also represent internal local inventory backends that scan `data/models/llms`, `data/models/embeddings`, and `data/models/rerankers` without loading model weights; internal Provider Profile detail pages show the local model environment, compact install commands, and local runtime device or GPU layer settings. LLM Model Profiles hold user-facing model selection and behavior defaults such as provider model id, stable profile key, capabilities, generation defaults, notes, and enabled state. Internal LLM Model Profiles use `llm/...` refs from `data/models/llms`; Embedding Model Profiles can use Provider Profiles with `embedding/...` refs or external embedding APIs; Reranker Model Profiles use internal Provider Profiles with `reranker/...` refs from `data/models/rerankers`.
...
Knowledge RAG v1 provides embedding and reranker model profiles, Knowledge Bases, source indexing, retrieval search, session bindings, automatic context injection, and a thin Knowledge Capability. Knowledge Defaults can leave reranking disabled or select an enabled Reranker Model Profile while keeping retrieval strategy parameters in Knowledge settings. Full Knowledge contract: [docs/contracts/knowledge.md](docs/contracts/knowledge.md).
...
Settings -> Models -> Embedding Model Profiles owns embedding profile management, including Provider Profile selection for internal embeddings, OpenAI-compatible/LM Studio embeddings, and Ollama embed. Settings -> Models -> Reranker Model Profiles owns internal reranker model selection. Settings -> Knowledge owns local RAG defaults, Knowledge Bases, and source indexing status. The Chat header Context Sources modal manages session Knowledge Base and Worldbook bindings. KB binding order is preserved for UI continuity but does not change Knowledge retrieval ranking.
...
Settings -> Capabilities -> Web Search controls `/web-search <query>` for a user-running SearXNG service, defaulting to `http://localhost:8888`. The Settings detail page can test the current SearXNG connection and search query using the current draft config without saving it. The same Capability settings support domain allowlist/blocklist filters and URL/title de-duplication for `/web-search`, test search, and Prompt Agent Web Context. `/web-search` returns one normalized JSON Message Part with `kind: "web_search_results"`, `schema: "web_search.results.v1"`, query, provider, timestamp, final results, warnings, and compact diagnostics. It does not fetch result page bodies, cache results, save to Knowledge, or run Prompt Agent context injection through the command path. Settings -> General -> Web Search controls whether eligible Prompt Agent runs reuse the same SearXNG search helper for `# Retrieved Web` and whether those runs may progressively fetch and optionally gate compact page excerpts.
