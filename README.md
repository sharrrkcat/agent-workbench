...
Web Context can optionally fetch retained result pages progressively as compact plain-text excerpts for Prompt Agent context only. Page fetching is best-effort, supports HTTP/HTTPS HTML without JavaScript or browser rendering, and does not affect `/web-search`, Settings test search, Knowledge storage, or vector indexes.
