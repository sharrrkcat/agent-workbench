...
Settings -> General -> Web Search controls whether ordinary Prompt Agent messages inject `# Retrieved Web` during context building. The Chat composer Web Search toggle updates that same global setting. When enabled, the runtime searches the current user input through the Web Search Capability's SearXNG helper and injects compact result titles, URLs, domains, dates, and snippets; Settings -> Capabilities -> Web Search still owns the SearXNG provider connection and `/web-search` command configuration.
