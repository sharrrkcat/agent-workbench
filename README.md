...
The Utility LLM JSON extractor may add compact slots such as `kb_hint` or `query` after the semantic prediction, especially for `knowledge_query`. It is no longer the primary classifier. Its prompt receives only compact intent examples, Knowledge Base names/aliases, and Agent names/aliases/examples, not Agent prompts, KB content, Worldbook content, or raw run history. In `shadow` mode it still never changes routing. Auto execution remains deliberately narrow:

- `chat` keeps the current Prompt Agent path and does not add a temporary Knowledge override.
- `knowledge_query` keeps the current Prompt Agent path but may provide a per-run temporary Knowledge Base/query override. This does not persist Context Sources bindings and does not change retrieval ranking.
- `pet_command` may execute only the existing `/pet` Capability command path for status, wake, tuck, select, or reload. Pet candidates come from the existing Pet capability settings/list data. Wake recognizes narrow Workbench Pet phrases such as `召唤`, `唤出`, `叫出来`, `叫醒`, `summon`, `bring out`, and `show pet`. `/pet random` is not implemented.
