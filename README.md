...
- `pet_command` may execute only the existing `/pet` Capability command path for status, wake, tuck, select, or reload. Pet candidates come from the existing Pet capability settings/list data. `/pet random` is not implemented.
...
Route Test reports `would_execute`, `route_action`, semantic thresholds used, grouped intent scores, and a readable not-executed reason such as score below threshold, margin below threshold, no KB candidate, pet candidate ambiguous, or diagnostic-only. For `pet_command`, it also reports pet action, target pet, optional source pet, and generated `/pet ...` command. Explicit `/command`, `@agent`, `@agent:action`, and `:action` inputs always bypass Intent Routing, so explicit `/pet wake` remains a normal slash command. `command_like` intents are recorded but are not executed automatically, so requests such as freeing memory do not run `/free-memory` in this version. Auto routing does not change the session default Agent, the visible Agent selector, Pet settings except through the executed `/pet` command itself, or Context Sources Knowledge bindings. There is no raw embedding path setting; the semantic router only accepts an existing enabled Knowledge Embedding Model Profile and never downloads models automatically.
