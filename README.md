...
Settings -> Capabilities -> Web Search controls `/web-search <query>` for a user-running SearXNG service, defaulting to `http://localhost:8888`. The Settings detail page can test the current SearXNG connection and search query using the current draft config without saving it. `/web-search` returns one normalized JSON Message Part with `kind: "web_search_results"`, `schema: "web_search.results.v1"`, query, provider, timestamp, results, and warnings. It does not fetch result page bodies, cache results, save to Knowledge, or run Prompt Agent context injection through the command path. Settings -> General -> Web Search controls whether eligible Prompt Agent runs reuse the same SearXNG search helper for `# Retrieved Web`.
