...
Web Context can also optionally use the Utility LLM as a conservative noise filter before page fetching. It may remove clearly unhelpful candidates, but uncertain or unjudged candidates are kept; the Utility LLM does not independently decide all Web sources. This best-effort judge uses strict JSON, stores only compact diagnostics, and falls back to normal filtered results if the Utility LLM is unavailable or returns invalid JSON.
